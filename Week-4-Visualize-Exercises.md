...
``` r
ggplot(data = mpg) +
  geom_point(mapping = aes(x = displ, y = hwy, color = class, shape = drv, size = trans))+
  labs(x = "Engine Size Displacement",
       y = "Highway Mileage",
       Color = "Class of Vehicle",
       shape = "Drive Type",
       size = "Transmission Type",
       title = "The Bigger the Engine, the Less Milage")
```
...
Q: What will this code do? A: This code will create a scatter plot with
engine size on the x axis and mileage on the y axis. geom_smooth will
add a smoothed trend line, and then the graph is saved as a jpg.
